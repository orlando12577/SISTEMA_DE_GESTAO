...
<div class="flex justify-center items-center min-h-screen bg-gray-900">
    <div class="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-lg text-gray-900">
        ...
        <form action="{{ route('unidades.update', $unidade) }}" method="POST" class="space-y-5 text-gray-900">
            @csrf
            @method('PUT')

            <div>
                <label for="nome_fantasia" class="block font-semibold mb-1 text-gray-900">Nome Fantasia</label>
            ...
            <div>
                ...
                <select name="bandeira_id" id="bandeira_id"
                    ...
                    @foreach($bandeiras as $bandeira)
                        <option value="{{ $bandeira->id }}" {{ old('bandeira_id', $unidade->bandeira_id) == $bandeira->id ? 'selected' : '' }}>
                            {{ $bandeira->nome }}
                        </option>
                    @endforeach
                </select>
